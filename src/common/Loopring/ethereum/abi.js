...
function generateMultisendTokenData(token,address, amount) {
  // validator.validate({value: address, type: 'ADDRESS'});
  // validator.validate({value: amount, type: 'QUANTITY'});
  const method = methodID('multisendToken', ['address','address[]', 'uint256[]']).toString('hex');
  const data = rawEncode(['address','address[]' ,'uint256[]'], [token,address, amount]).toString('hex');
  return '0x' + method + data;
}
...
export function generateAbiData({method, timestamp, address, amount, order, owner, spender, projectId, tokenA, tokenB, key,name,signer,feeRecipient}) {
  ...
  if (method === 'multisendToken') {
    console.log(tokenA,address, amount)
    return generateMultisendTokenData(tokenA,address, amount);
  }
  if (method === 'balanceOf') {
