...
import {getAssets} from 'Loopring/relay/genv';
...
class ListSidebar extends React.Component {
  ...
  async componentDidMount() {
   ...
   await getAssets(window.WALLET.getAddress(),window.STORAGE.settings.get().preference.network).then(res => {

      if (!res.error) {
        this.setState({tokens: res})
        localStorage.genv_tokens=JSON.stringify(res);
      }else{
        alert("获取账户token失败")
      }
    })
  ...
  render() {
   ...
   let items=this.state.tokens;
    ...
    const gotoBatchTransfer = (item, e) => {
      e.stopPropagation()
      showModal({
    ...
    const TokenItemActions = (token) => (
      <div style={{minWidth: '150px', maxWidth: '250px'}}>
        <div className="row no-gutters p5">
          ...
          <div className="col-12 p5">
            <Button onClick={gotoBatchTransfer.bind(this, token)} className="d-block w-100 text-left" type="primary">
              <i className="icon icon-loopring icon-loopring-receive fs16 mr5"/>
              {intl.get('tokens.options_batchtransfer')} {token.symbol}
            </Button>
          </div>
